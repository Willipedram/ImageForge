...
def test_preflight_logs_professional_discovery_summary(tmp_path, caplog):
    ...
    assert '"diagnosis_code": "FTP_DOCUMENT_ROOT_EMPTY"' in summary
...
def test_preflight_logs_the_exact_failed_keypoint(tmp_path, caplog):
    ...
    assert '"run":' in caplog.text and '"root": "/"' in caplog.text


def test_keypoint_context_is_structured_and_redacts_secrets(caplog):
    with caplog.at_level("INFO"):
        log_keypoint(logging.getLogger("keypoint-test"), "upload", "stopped", context={
            "path": "/public_html/image.jpg", "detail": "password=do-not-log",
        })
    assert '"path": "/public_html/image.jpg"' in caplog.text
